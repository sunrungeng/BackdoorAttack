...
def ImageTrain(helper, start_epoch, local_model, target_model, is_poison, agent_name_keys):
    ...
    if helper.params['attack_flag'] == 1:
        ...
        for adversary_key in helper.params['adversary_list']:
            attack_clean_grad = []
            epochs_attack_clean_local_update = []
            model = local_model
            ...
            for epoch in range(start_epoch, start_epoch + helper.params['aggr_epoch_interval']):
                ...
                for internal_epoch in range(1, helper.params['internal_epochs'] + 1):
                    temp_local_epoch += 1
                    _, data_iterator = helper.train_data[adversary_key]
                    total_loss = 0.
                    correct = 0
                    dataset_size = 0
                    dis2global_list = []
                    for batch_id, batch in enumerate(data_iterator):
                        ...
                        if helper.params['aggregation_methods'] == config.AGGR_FOOLSGOLD \
                                or helper.params['aggregation_methods'] == config.AGGR_MULTI_KRUM \
                                or helper.params['aggregation_methods'] == config.AGGR_TRIMMED_MEAN \
                                or helper.params['aggregation_methods'] == config.AGGR_BULYAN:
                            for i, (name, params) in enumerate(model.named_parameters()):
                                if params.requires_grad:
                                    if internal_epoch == 1 and batch_id == 0:
                                        attack_clean_grad.append(params.grad.clone())
                                    else:
                                        attack_clean_grad[i] += params.grad.clone()
                        ...
                        if helper.params["vis_train_batch_loss"]:
                            ...
                            model.train_batch_vis(vis=main.vis,
                                                  ...
                                                  name=str(adversary_key), win='train_batch_loss', is_poisoned=False)
                        if helper.params["batch_track_distance"]:
                            ...
                            model.track_distance_batch_vis(vis=main.vis, epoch=temp_local_epoch,
                                                           ...
                                                           eid=helper.params['environment_name'],
                                                           name=str(adversary_key), is_poisoned=False)
                    acc = 100.0 * (float(correct) / float(dataset_size))
                    total_l = total_loss / dataset_size
                    main.logger.info(
                        '___adversary_clean___Train {},  epoch {:3d}, local model {}, internal_epoch {:3d},  Average loss: {:.4f}, '
                        '___adversary_clean___Accuracy: {}/{} ({:.4f}%)'.format(model.name, epoch, adversary_key,
                                                                                internal_epoch,
                                                                                total_l, correct, dataset_size,
                                                                                acc))
                    csv_record.train_result.append([adversary_key, temp_local_epoch,
                    ...
                    if helper.params['vis_train']:
                        model.train_vis(main.vis, temp_local_epoch,
                                        ...
                                        name=str(adversary_key))
                    num_samples_dict[adversary_key] = dataset_size

                    if helper.params["batch_track_distance"]:
                        main.logger.info(
                            f'MODEL {adversary_key}. P-norm is {helper.model_global_norm(model):.4f}. '
                ...
                # 如果是以下几种聚合方式，更新梯度；否则更新参数
                if helper.params['aggregation_methods'] == config.AGGR_FOOLSGOLD \
                        or helper.params['aggregation_methods'] == config.AGGR_MULTI_KRUM \
                        or helper.params['aggregation_methods'] == config.AGGR_TRIMMED_MEAN \
                        or helper.params['aggregation_methods'] == config.AGGR_BULYAN:
                    epochs_attack_clean_local_update.append(attack_clean_grad)
                else:
                    epochs_attack_clean_local_update.append(local_model_update_dict)
            attack_clean_update_dict[adversary_key] = epochs_attack_clean_local_update
    ...
    for model_id in range(helper.params['no_models']):
        ...
        for epoch in range(start_epoch, start_epoch + helper.params['aggr_epoch_interval']):
            ...
            # 需要进行中毒攻击、当前聚合者是攻击者、当前epoch是需要进行攻击的epoch
            if is_poison and agent_name_key in helper.params['adversary_list'] and (epoch in localmodel_poison_epochs):
                ...
                for internal_epoch in range(1, internal_epoch_num + 1):
                    ...
                    dis2global_list = []
                    for batch_id, batch in enumerate(data_iterator):
                        data, targets, poison_num = helper.get_poison_batch(batch, adversarial_index=adversarial_index,
                                                                            evaluation=False)
